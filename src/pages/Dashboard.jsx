...
// Small reusable lock-icon badge, used to mark Pro-only features for
// free users. Clicking anywhere on it opens the shared upgrade modal.
export const ProLockBadge = ({ onClick, label = "Pro feature" }) => (
  <button
    onClick={onClick}
    className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-gray-100 hover:bg-yellow-50 border border-gray-200 hover:border-yellow-300 rounded-full text-xs font-bold text-gray-500 hover:text-yellow-700 transition"
    aria-label={`${label} — upgrade to Pro to unlock`}
  >
    <svg
...
const PaymentSuccessView = ({ onContinue, searchParams }) => {
  ...
  useEffect(() => {
    ...
    const verifySession = async () => {
      ...
      try {
        const res = await fetch(
          `http://localhost:8000/api/checkout/verify-session?session_id=${encodeURIComponent(sessionId)}`,
        );
        if (!res.ok) throw new Error("Verification failed");
        const data = await res.json();
        setVerified(Boolean(data?.paid));
...
const Dashboard = () => {
  ...
  const user = useMemo(
    () => ({
      name: profile?.full_name || "Muhammad Junaid",
      email: authUser?.email || "junaid@example.com",
      role: "Full-Stack Developer",
      plan: isProUser ? "Pro" : "Free Tier",
      joinDate: "August 2025",
      nextBillingDate: profile?.next_billing_date || "August 22, 2026",
  ...
  const handleCancelSubscription = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        "http://localhost:8000/api/billing/cancel-subscription",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    try {
      const res = await fetch(
        "http://localhost:8000/api/checkout/create-session",
        {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            userId: authUser.id, // lets the Stripe webhook know whose profile to upgrade
            successUrl: `${window.location.origin}/payment/success`,
            cancelUrl: `${window.location.origin}/payment/cancel`,
          }),
        },
    ...
    <div className="min-h-screen bg-[#EFF6FF] flex relative">
      ...
      <main className="flex-1 flex flex-col p-4 sm:p-6 lg:p-8 overflow-y-auto relative">
        ...
        <header className="flex justify-between items-center mb-8 border-b border-gray-200 pb-4 min-h-[76px]">
          ...
          <div className="flex items-center gap-3">
            {isProUser && (
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-black tracking-wide bg-green-100 text-green-700 border border-green-200">
                PRO
              </span>
            )}
            <button
            ...
            <div className="space-y-8">
              ...
              <div className="relative min-h-[156px] sm:min-h-[144px] rounded-2xl overflow-hidden">
                {isLoading ? (
                  ...
                  <div className="absolute inset-0 bg-gradient-to-r from-[#1E3A5F] to-[#1A56DB] rounded-2xl p-6 sm:p-8 text-white shadow-lg flex justify-between items-center flex-wrap gap-4 overflow-y-auto">
                    <div>
                      <h2 className="text-2xl sm:text-4xl font-black flex items-center gap-3 flex-wrap">
                        Welcome back, {user.name}!
                        {isProUser && (
